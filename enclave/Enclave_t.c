...
typedef struct ms_seal_encrypted_data_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_cipher_text;
	size_t ms_cipher_text_len;
	uint8_t* ms_sealed_cipher_text;
	size_t ms_sealed_cipher_text_len;
	int64_t* ms_sealed_length_for_trimming;
} ms_seal_encrypted_data_t;

typedef struct ms_unseal_aes_encrypt_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_sealed_data_received;
	size_t ms_sealed_data_received_len;
	uint8_t* ms_aes_encrypted_placeholder;
	size_t ms_aes_encrypted_placeholder_len;
	int64_t* ms_aes_encrypted_length;
} ms_unseal_aes_encrypt_t;
...
static sgx_status_t SGX_CDECL sgx_seal_encrypted_data(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_seal_encrypted_data_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_seal_encrypted_data_t* ms = SGX_CAST(ms_seal_encrypted_data_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_cipher_text = ms->ms_cipher_text;
	size_t _tmp_cipher_text_len = ms->ms_cipher_text_len;
	size_t _len_cipher_text = _tmp_cipher_text_len;
	uint8_t* _in_cipher_text = NULL;
	uint8_t* _tmp_sealed_cipher_text = ms->ms_sealed_cipher_text;
	size_t _tmp_sealed_cipher_text_len = ms->ms_sealed_cipher_text_len;
	size_t _len_sealed_cipher_text = _tmp_sealed_cipher_text_len;
	uint8_t* _in_sealed_cipher_text = NULL;
	int64_t* _tmp_sealed_length_for_trimming = ms->ms_sealed_length_for_trimming;
	size_t _len_sealed_length_for_trimming = sizeof(int64_t);
	int64_t* _in_sealed_length_for_trimming = NULL;

	CHECK_UNIQUE_POINTER(_tmp_cipher_text, _len_cipher_text);
	CHECK_UNIQUE_POINTER(_tmp_sealed_cipher_text, _len_sealed_cipher_text);
	CHECK_UNIQUE_POINTER(_tmp_sealed_length_for_trimming, _len_sealed_length_for_trimming);
	...
	if (_tmp_cipher_text != NULL && _len_cipher_text != 0) {
		if ( _len_cipher_text % sizeof(*_tmp_cipher_text) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_cipher_text = (uint8_t*)malloc(_len_cipher_text);
		if (_in_cipher_text == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_cipher_text, _len_cipher_text, _tmp_cipher_text, _len_cipher_text)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_sealed_cipher_text != NULL && _len_sealed_cipher_text != 0) {
		if ( _len_sealed_cipher_text % sizeof(*_tmp_sealed_cipher_text) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_sealed_cipher_text = (uint8_t*)malloc(_len_sealed_cipher_text)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_sealed_cipher_text, 0, _len_sealed_cipher_text);
	}
	if (_tmp_sealed_length_for_trimming != NULL && _len_sealed_length_for_trimming != 0) {
		if ( _len_sealed_length_for_trimming % sizeof(*_tmp_sealed_length_for_trimming) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_sealed_length_for_trimming = (int64_t*)malloc(_len_sealed_length_for_trimming)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_sealed_length_for_trimming, 0, _len_sealed_length_for_trimming);
	}

	ms->ms_retval = seal_encrypted_data((const uint8_t*)_in_cipher_text, _tmp_cipher_text_len, _in_sealed_cipher_text, _tmp_sealed_cipher_text_len, _in_sealed_length_for_trimming);
	if (_in_sealed_cipher_text) {
		if (memcpy_s(_tmp_sealed_cipher_text, _len_sealed_cipher_text, _in_sealed_cipher_text, _len_sealed_cipher_text)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_in_sealed_length_for_trimming) {
		if (memcpy_s(_tmp_sealed_length_for_trimming, _len_sealed_length_for_trimming, _in_sealed_length_for_trimming, _len_sealed_length_for_trimming)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_cipher_text) free(_in_cipher_text);
	if (_in_sealed_cipher_text) free(_in_sealed_cipher_text);
	if (_in_sealed_length_for_trimming) free(_in_sealed_length_for_trimming);
	return status;
}

static sgx_status_t SGX_CDECL sgx_unseal_aes_encrypt(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_unseal_aes_encrypt_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_unseal_aes_encrypt_t* ms = SGX_CAST(ms_unseal_aes_encrypt_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_sealed_data_received = ms->ms_sealed_data_received;
	size_t _tmp_sealed_data_received_len = ms->ms_sealed_data_received_len;
	size_t _len_sealed_data_received = _tmp_sealed_data_received_len;
	uint8_t* _in_sealed_data_received = NULL;
	uint8_t* _tmp_aes_encrypted_placeholder = ms->ms_aes_encrypted_placeholder;
	size_t _tmp_aes_encrypted_placeholder_len = ms->ms_aes_encrypted_placeholder_len;
	size_t _len_aes_encrypted_placeholder = _tmp_aes_encrypted_placeholder_len;
	uint8_t* _in_aes_encrypted_placeholder = NULL;
	int64_t* _tmp_aes_encrypted_length = ms->ms_aes_encrypted_length;
	size_t _len_aes_encrypted_length = sizeof(int64_t);
	int64_t* _in_aes_encrypted_length = NULL;

	CHECK_UNIQUE_POINTER(_tmp_sealed_data_received, _len_sealed_data_received);
	CHECK_UNIQUE_POINTER(_tmp_aes_encrypted_placeholder, _len_aes_encrypted_placeholder);
	CHECK_UNIQUE_POINTER(_tmp_aes_encrypted_length, _len_aes_encrypted_length);
	...
	if (_tmp_sealed_data_received != NULL && _len_sealed_data_received != 0) {
		if ( _len_sealed_data_received % sizeof(*_tmp_sealed_data_received) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_sealed_data_received = (uint8_t*)malloc(_len_sealed_data_received);
		if (_in_sealed_data_received == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_sealed_data_received, _len_sealed_data_received, _tmp_sealed_data_received, _len_sealed_data_received)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_aes_encrypted_placeholder != NULL && _len_aes_encrypted_placeholder != 0) {
		if ( _len_aes_encrypted_placeholder % sizeof(*_tmp_aes_encrypted_placeholder) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_aes_encrypted_placeholder = (uint8_t*)malloc(_len_aes_encrypted_placeholder)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_aes_encrypted_placeholder, 0, _len_aes_encrypted_placeholder);
	}
	if (_tmp_aes_encrypted_length != NULL && _len_aes_encrypted_length != 0) {
		if ( _len_aes_encrypted_length % sizeof(*_tmp_aes_encrypted_length) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_aes_encrypted_length = (int64_t*)malloc(_len_aes_encrypted_length)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_aes_encrypted_length, 0, _len_aes_encrypted_length);
	}

	ms->ms_retval = unseal_aes_encrypt((const uint8_t*)_in_sealed_data_received, _tmp_sealed_data_received_len, _in_aes_encrypted_placeholder, _tmp_aes_encrypted_placeholder_len, _in_aes_encrypted_length);
	if (_in_aes_encrypted_placeholder) {
		if (memcpy_s(_tmp_aes_encrypted_placeholder, _len_aes_encrypted_placeholder, _in_aes_encrypted_placeholder, _len_aes_encrypted_placeholder)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_in_aes_encrypted_length) {
		if (memcpy_s(_tmp_aes_encrypted_length, _len_aes_encrypted_length, _in_aes_encrypted_length, _len_aes_encrypted_length)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_sealed_data_received) free(_in_sealed_data_received);
	if (_in_aes_encrypted_placeholder) free(_in_aes_encrypted_placeholder);
	if (_in_aes_encrypted_length) free(_in_aes_encrypted_length);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_seal_encrypted_data, 0, 0},
		{(void*)(uintptr_t)sgx_unseal_aes_encrypt, 0, 0},
