...
sgx_status_t seal_encrypted_data(sgx_enclave_id_t eid, sgx_status_t* retval, const uint8_t* cipher_text, size_t cipher_text_len, uint8_t* sealed_cipher_text, size_t sealed_cipher_text_len, int64_t* sealed_length_for_trimming)
{
	sgx_status_t status;
	ms_seal_encrypted_data_t ms;
	ms.ms_cipher_text = cipher_text;
	ms.ms_cipher_text_len = cipher_text_len;
	ms.ms_sealed_cipher_text = sealed_cipher_text;
	ms.ms_sealed_cipher_text_len = sealed_cipher_text_len;
	ms.ms_sealed_length_for_trimming = sealed_length_for_trimming;
	status = sgx_ecall(eid, 0, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t unseal_aes_encrypt(sgx_enclave_id_t eid, sgx_status_t* retval, const uint8_t* sealed_data_received, size_t sealed_data_received_len, uint8_t* aes_encrypted_placeholder, size_t aes_encrypted_placeholder_len, int64_t* aes_encrypted_length)
{
	sgx_status_t status;
	ms_unseal_aes_encrypt_t ms;
	ms.ms_sealed_data_received = sealed_data_received;
	ms.ms_sealed_data_received_len = sealed_data_received_len;
	ms.ms_aes_encrypted_placeholder = aes_encrypted_placeholder;
	ms.ms_aes_encrypted_placeholder_len = aes_encrypted_placeholder_len;
	ms.ms_aes_encrypted_length = aes_encrypted_length;
